            ...
            <!-- フィルタリングフォームとソートリンク -->
                    <div class="flex items-center space-x-4 mb-4">
                        <form method="GET" action="{{ route('catalogs.index') }}" id="filterForm" class="flex items-center space-x-4">
                            <div>
                                <select name="country_name" id="country_name" class="mt-4 block w-full border-gray-300 rounded-md shadow-sm text-sm" onchange="document.getElementById('filterForm').submit();">
                            ...
                            <div>
                                <select name="location_name" id="location_name" class="mt-4 block w-full border-gray-300 rounded-md shadow-sm text-sm" onchange="document.getElementById('filterForm').submit();">
                            ...
                            <div>
                                <select name="status_id" id="status_id" class="mt-4 block w-full border-gray-300 rounded-md shadow-sm text-sm" onchange="document.getElementById('filterForm').submit();">
                            ...
                            <div class="block mb-4 p-4 bg-white shadow-sm rounded-md">
                                ...
                                <div class="flex items-center justify-between text-gray-700 mt-2">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <a href="{{ route('catalogs.show', $catalog) }}" class="px-6 py-2 text-white rounded-md bg-gradient-to-r from-emerald-500 to-sky-600 hover:from-emerald-300 hover:to-sky-400 hover:shadow-sm ">
                                            詳細はこちら
                                        </a>
                                    </div>
                                </div>
                                ...
                                <span class="text-xs text-gray-700">所在地:{{ $catalog->provider->location ?? '不明' }}
                                </span>
                                <span class="text-xs text-gray-700">TEL.{{ $catalog->provider->phone_number ?? '不明' }}
                                </span>
                                <span class="text-xs text-gray-700">FAX.{{ $catalog->provider->fax_number ?? '不明' }}
                                </span>
                            </div>
                        @endforeach
                    @else
